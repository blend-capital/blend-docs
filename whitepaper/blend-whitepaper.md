...
$$
\tag{1} bTokens = \frac{amount}{bTokenLenderRate}
$$
...
$$
\tag{2} dTokens=\frac{amount}{dTokenRate}
$$
...
$$
\tag{3} Borrow Limit = \sum(PositionValue_{collateral} * AssetFactor_{collateral}) -\sum(\frac{PositionValue_{Liability}}{AssetFactor_{Liability}})
$$
...
$$
\tag{4} U=1- \frac{Balance_{Pool}}{bTokenTotalSupply * bTokenRate}
$$
...
$$
\tag{5} IR(U)= \begin{cases} RM*(R_{base}+\frac{U}{U_T}R_1) & \text{if } U\leq U_T\\ RM*(R_{base}+R_1+\frac{U-U_T}{0.95-U_T}R_2) & \text{if } U_T\lt U\leq 0.95\\ RM*(R_{base}+R_1+R_2)+\frac{U-0.95}{0.05}R_3 & \text{if } 0.95\lt U\\ \end{cases}
$$
...
$$
\tag{6} Rate Modifier = Util Rate Error * Reactivity Constant + Rate Modifier
$$
...
$$
\tag{7} Loan Value = Loan Value * (1 + R * \frac{\Delta blocks}{Blocks Per Year})
$$
...
$$
\tag{8} TLA=\frac{1.03 * L_o - C_o}{1.03 * \overline{L_f}^{(-1)} - p\overline{C_f}}
$$
...
$$
\tag{9} LM(\Delta b)= \begin{cases} \lfloor \frac{\Delta b}{2} \rfloor & \text{if } \Delta b\leq 200\\ 1 & \text{if } \Delta b\gt 200\\ \end{cases}, \hspace{2mm} BM(\Delta b)= \begin{cases} 1 & \text{if } \Delta b\leq 200\\ \lfloor \frac{\Delta b}{2} \rfloor& \text{if } \Delta b\gt 200\\ \end{cases}
$$
